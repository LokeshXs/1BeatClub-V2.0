...
const LightRay = React.memo<LightRayProps>(
  ({
    ...
    return (
      <motion.div
        className="absolute pointer-events-none -top-[5%] left-[var(--ray-left)] w-[var(--ray-width)] h-[var(--ray-height)] origin-top mix-blend-screen bg-[linear-gradient(to_bottom,rgba(255,95,151,var(--ray-opacity)),rgba(255,95,151,0))] blur-[var(--ray-blur)] translate-x-[-50%] rotate-[var(--ray-rotation)]"
        style={
          {
            "--ray-left": left,
            "--ray-width": `${width}px`,
...
export const GridBeams: React.FC<GridBeamsProps> = ({
  ...
  return (
    <div
      className={cn(
        "relative overflow-hidden bg-[var(--bg-color)] bg-[radial-gradient(ellipse_at_50%_0%,#ff5f9733,transparent_70%)]",
        className,
      )}
      style={
      {
    ...
    >
      ...
      <motion.div initial={{opacity:0,y:-20}} animate={{opacity:1,y:0}} transition={{duration:1,delay:1.5}} className="absolute inset-0 overflow-hidden pointer-events-none">
